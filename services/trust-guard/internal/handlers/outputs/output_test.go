...
type outputAnalyzerResponse struct {
...
func TestOutputAnalyzerMaliciousContent(t *testing.T) {
	rules := testDetectionRules()
	policyConfig := testPolicyConfig()

	handler := OutputAnalyzer(&rules, &policyConfig)
	...
	var response outputAnalyzerResponse
