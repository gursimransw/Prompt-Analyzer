...
func OutputAnalyzer(detectionRules *[]types.DetectionRule, policyConfig *types.PolicyConfig) http.HandlerFunc { //This is the API layer for the function
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		slog.Info("Analyzing the Prompt....", slog.String("prompt", logic.MaskRedactedValues(output.Content)), slog.String("requestId", requestId))

		detectionRuleMatched, matchedRules, matchedRulesCategories, matchedRulesReasons, matchedRulesEffectiveWeight, effectiveSeverity, effectiveActions, findings, scanContext := logic.AnalyzeContent(detectionRules, policyConfig, output.Content, "output")
